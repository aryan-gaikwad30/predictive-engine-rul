...
export default function ResultsView({ results }: { results: PredictionResponse }) {
  ...
  const latestRul = (chartData.length > 0 ? chartData[chartData.length - 1].predicted_rul : 0) as number;
  const healthStatus = latestRul <= 30 ? "CRITICAL" : latestRul <= 75 ? "WARNING" : "HEALTHY";
  const healthColor = latestRul <= 30 ? "text-red-600" : latestRul <= 75 ? "text-[var(--color-industrial)]" : "text-green-600";
  const healthBg = latestRul <= 30 ? "bg-red-50 border-red-200" : latestRul <= 75 ? "bg-orange-50 border-orange-200" : "bg-green-50 border-green-200";
      ...
      <section className="py-24 relative overflow-hidden">
        <div className="container mx-auto px-6 max-w-6xl relative z-10">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-16 border-t-2 border-[var(--color-border)] pt-16">
             ...
             <MetricBlock label="NASA Score" value={typeof metrics.NASA_score === 'number' ? metrics.NASA_score.toFixed(0) : metrics.NASA_score} />
      ...
      <section className="py-24 bg-[var(--color-offwhite)]">
        <div className="container mx-auto px-6 max-w-7xl">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 gap-8">
            <h3 className="text-3xl md:text-4xl font-bold uppercase tracking-tighter text-[var(--color-graphite)]">
          ...
          <div className={`mb-8 p-6 rounded-3xl border ${healthBg} flex flex-col md:flex-row justify-between items-center gap-6 transition-colors duration-500`}>
             ...
             <div className="flex items-baseline gap-4">
               <div className={`text-5xl font-bold tracking-tighter ${healthColor}`}>
                 {Math.max(0, Math.round(latestRul))}
               </div>
          ...
          <div className="h-[500px] w-full bg-white rounded-[2rem] p-8 shadow-sm border border-[var(--color-border)]">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData} margin={{ top: 20, right: 30, left: -20, bottom: 0 }}>
                <XAxis dataKey={timeCol} stroke="var(--color-muted)" tick={{fontSize: 12, fontWeight: 'bold'}} axisLine={false} tickLine={false} />
