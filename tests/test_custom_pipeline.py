...
def test_pipeline_end_to_end_and_shapes():
    ...
    # Fleet Predictions
    assert len(result.fleet_predictions) == len(df)
    assert "split" in result.fleet_predictions.columns
    assert set(result.fleet_predictions["split"].unique()) == {"training", "validation"}
    assert result.metadata["total_machine_count"] == df["machine_id"].nunique()
...
def test_pipeline_entity_preservation_and_disjoint():
    ...
    # Fleet check
    fleet_entities = set(result.fleet_predictions["unit"].unique())
    assert len(fleet_entities) == 10
