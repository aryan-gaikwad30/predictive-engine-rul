...
def get_prediction_diagnostics(
    ...
    Args:
        ...
        target: The target column name (e.g. 'RUL_clipped') used for training

    Returns:
        A DataFrame with engine/unit, cycle, actual_RUL, actual_RUL_clipped, predicted_RUL, and error.
    """
    df = pd.DataFrame()
    df["unit"] = validation_df["unit"]
    df["cycle"] = validation_df["cycle"]
    df["actual_RUL"] = validation_df["RUL"]
    df["actual_RUL_clipped"] = validation_df["RUL_clipped"]
    df["predicted_RUL"] = y_pred
    df["error"] = y_pred - validation_df["RUL"]
...
def run_baseline_models(
    ...
    try:
        ...
        y_validation_raw = validation_df["RUL"]

        rf_rmse = rmse_score(y_validation_raw, rf_preds)
        rf_nasa = nasa_phm08_score(y_validation_raw, rf_preds)
    ...
    try:
        ...
        xgb_rmse = rmse_score(y_validation_raw, xgb_preds)
        xgb_nasa = nasa_phm08_score(y_validation_raw, xgb_preds)
